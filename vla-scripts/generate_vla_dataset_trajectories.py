...
#            return self.obs["robot0_eef_quat"].copy()
    ...
    def _log_step(self, action, traj_buffer):
        """
        Append the current step to traj_buffer (if provided).

        action: np.array, shape (7,) - [x,y,z,rotvec(3),gripper]
        """
        if traj_buffer is None:
            return

        # ---- 1. Get per-timestep RGB image ----
        # robosuite puts camera obs in e.g. "agentview_image"
        if "agentview_image" in self.obs:
            rgb = self.obs["agentview_image"]
        else:
            # fallback: render directly from sim
            H, W = self.cam_height, self.cam_width
            ...
            # match orientation of normal camera obs (no BGR conversion here)
            rgb = np.flipud(rgb)

        if rgb.dtype != np.uint8:
            rgb = np.clip(rgb, 0, 255).astype(np.uint8)

        # ---- 2. Log everything into the trajectory buffer ----
        traj_buffer.append({
            "action": action.copy(),                         # (7,)
            "eef_pos": self.obs["robot0_eef_pos"].copy(),    # (3,)
            "eef_quat": self._get_current_quat().copy(),     # (4,)
            "rgb": rgb.copy(),                               # (H, W, 3)
            # optional: explicit gripper scalar if you ever want it
    ...
    def move_to_pose(self, target_pos, target_quat, gripper, count,
                     time_for_residual_movement=10, traj_buffer=None):
        """
        Moves the robot to the target pose in a straight line using Slerp.
        Optionally logs each env.step into traj_buffer.
        """
        ...
        for i in range(1, count + 1):
            ...
            self._log_step(action, traj_buffer)
        ...
        for _ in range(time_for_residual_movement):
            ...
            self.obs, _, _, _ = self.env.step(action)
            self._video_frame("Holding pose")
            self._log_step(action, traj_buffer)
    ...
    def move_gripper(self, gripper_action, count=20, traj_buffer=None):
        """
    ...
    def _video_start(self, path="eval.mp4", fps=30, H=256, W=256, camera_name="agentview"):
        self._rec = {"on": False, "path": path, "fps": fps, "H": H, "W": W, "camera": camera_name, "frames": 0}
    ...
    def save_sample(self, trial_idx, rgb_img, depth_img,
                instruction, waypoint_labels, trajectory):
        ...
        # --- 3. Save Metadata (instruction + affordance waypoints) ---
        metadata = {
            "instruction": instruction,
            "affordance_waypoints": waypoint_labels,   # A1, A2, A3, A4
        ...
        # --- 4. Save full trajectory ---
        if trajectory is not None and len(trajectory) > 0:
            # Stack everything from the raw logged steps
            actions = np.stack([step["action"]  for step in trajectory]).astype(np.float32)  # [T_raw, 7]
            eef_pos = np.stack([step["eef_pos"] for step in trajectory]).astype(np.float32)  # [T_raw, 3]
            eef_quat = np.stack([step["eef_quat"] for step in trajectory]).astype(np.float32)  # [T_raw, 4]
            images = np.stack([step["rgb"]      for step in trajectory]).astype(np.uint8)    # [T_raw, H, W, 3]

            # Build gripper array once from the trajectory (preferred),
            # or derive from actions[:, -1] if not logged explicitly.
            if "gripper" in trajectory[0]:
                gripper = np.array(
                    [step["gripper"] for step in trajectory],
                    dtype=np.float32,
                )  # [T_raw]
            else:
                gripper = actions[:, -1].astype(np.float32)  # [T_raw]

            # ----- Subsample or pad to fixed number of steps -----
            TARGET_STEPS = 50  # change to 64 or 80 if you prefer

            T = actions.shape[0]

            if T > TARGET_STEPS:
                # Uniformly subsample indices
                idxs = np.linspace(0, T - 1, TARGET_STEPS).astype(int)

                actions  = actions[idxs]
                eef_pos  = eef_pos[idxs]
                eef_quat = eef_quat[idxs]
                images   = images[idxs]
                gripper  = gripper[idxs]

            elif T < TARGET_STEPS:
                # Pad by repeating the last frame
                pad = TARGET_STEPS - T

                actions  = np.concatenate([actions,  actions[-1:].repeat(pad, axis=0)], axis=0)
                eef_pos  = np.concatenate([eef_pos,  eef_pos[-1:].repeat(pad, axis=0)], axis=0)
                eef_quat = np.concatenate([eef_quat, eef_quat[-1:].repeat(pad, axis=0)], axis=0)
                images   = np.concatenate([images,   images[-1:].repeat(pad, axis=0)], axis=0)
                gripper  = np.concatenate([gripper,  np.repeat(gripper[-1], pad)], axis=0)

            # At this point, all have shape [TARGET_STEPS, ...]
            np.savez(
                os.path.join(episode_dir, "trajectory.npz"),
                actions=actions,      # [TARGET_STEPS, 7]
                eef_pos=eef_pos,      # [TARGET_STEPS, 3]
                eef_quat=eef_quat,    # [TARGET_STEPS, 4]
                images=images,        # [TARGET_STEPS, H, W, 3]
                gripper=gripper,      # [TARGET_STEPS]
            )
    ...
    def execute_trajectory(self, trajectory_goals_3d):
        ...
        try:
            ...
            # --- Grasp sequence ---
            self.move_gripper(-1, count=20, traj_buffer=traj_buffer)

            print("  Moving to pre-grasp position...")
            self.move_to_pose(
                hover_pos_cereal, neutral_quat,
                gripper=-1.0, count=60, traj_buffer=traj_buffer
            )

            print("  Rotating to grasp orientation...")
            self.move_to_pose(
                hover_pos_cereal, grasp_quat,
                gripper=-1.0, count=40, traj_buffer=traj_buffer
            )

            print("  Descending to grasp...")
            self.move_to_pose(
                grasp_pos_cereal, grasp_quat,
                gripper=-1.0, count=50, traj_buffer=traj_buffer
            )

            print("  Closing gripper...")
            self.move_gripper(1, count=30, traj_buffer=traj_buffer)

            # --- LIFT & MOVE OVER BIN (this was missing) ---
            ...
            print("  Moving over bin...")
            self.move_to_pose(
                hover_pos_bin, place_quat,
                gripper=1.0, count=70, traj_buffer=traj_buffer
            )

            print("  Dropping object...")
            self.move_to_pose(
                place_pos_bin, place_quat,
                gripper=1.0, count=50, traj_buffer=traj_buffer
            )
            self.move_gripper(-1, count=20, traj_buffer=traj_buffer)
            ...
            self.move_to_pose(
                neutral_pos, neutral_quat,
                gripper=-1.0, count=60, traj_buffer=traj_buffer
            )

            print("Trajectory execution completed.")
            is_success = self.check_success(target_obj_name="Cereal")
            return is_success, traj_buffer

        except Exception as e:
            ...
            print(e)
            return False, traj_buffer
    ...
    def run_generation_loop(self, num_trials, num_videos, start_index=0):
        ...
        # 4. Get 3D poses and instruction
            ...
            is_success, trajectory = self.execute_trajectory(trajectory_goals_3d)
            ...
            if is_success:
                ...
                self.save_sample(i, rgb_image, depth_image, instruction, waypoint_labels, trajectory)
            else:
