...
Because the orientation is good but it barely misses the right edge, I will use the "Micro-Nudge XY" strategy. A tiny shift left (-0.02) or slightly larger shift (-0.03) will center A2.
    ...
    for i in range(cfg.num_times):
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                ...
                # Dynamically update the prompt text based on the attempt number
                if attempt == 0:
                    scenario_text = f"REAL SCENARIO - ATTEMPT 1\nBased on the rules and examples above, analyze this image and provide ONLY the JSON response.\nTrigger: {trigger_reason}"
                else:
                    scenario_text = f"REAL SCENARIO - ATTEMPT {attempt+1} (REFLECTION)\nWARNING: Your previous correction was applied, but the trajectory is STILL UNSAFE. The safety system triggered again:\nTrigger: {trigger_reason}\nLook closely at this newly updated image. Provide a NEW JSON correction to fix the remaining error."
                ...
                if not corrections:
                    ...
                    if osc_delta:
                        ...
                        orig_a1_quat = trajectory_goals_3d["A1_pregrasp"][1] # <-- 1. Fetch A1's original quaternion
                        
                        new_a1_pos = [new_a2_pos[0], new_a2_pos[1], orig_a1_pos[2]]
                        
                        # <-- 2. Save A1 with the new position, but its OWN original quaternion
                        trajectory_goals_3d["A1_pregrasp"] = (new_a1_pos, orig_a1_quat)
