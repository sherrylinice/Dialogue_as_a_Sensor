...
#            return False
    ...
    def run_generation_loop(self, num_trials, num_videos, start_index=0):
        ...
        for i in range(start_index, start_index + num_trials):
            ...
            # Reset env and robot state. Re-enable camera observables so
            # reset renders fresh image + depth.
            ...
            # Disable camera observables during trajectory steps to avoid
            # unnecessary GPU rendering on every step (saves memory and is
            # required when the GPU is constrained).
            for _obs_name in ("agentview_image", "agentview_depth"):
                if _obs_name in self.env._observables:
            ...
            # 3. Process Depth
            # Convert to meters. Replace NaN/inf and clip to [0, 1] to guard
            # against transient uninitialized depth-buffer values on EGL/GPU
            # after env.reset() (NVIDIA driver sometimes returns garbage on
            # the first frame after a context switch).
