...
class VideoEnv():
    ...
    def video_frame(self, text=None):
        ...
        # 1. NO MORE sim.render()! Steal the image directly from the observation.
        cam = self._rec["camera"]
        rgb = self.obs[f"{cam}_image"].copy()
        
        # 2. Process the image (Robosuite images are upside down)
    ...
    def video_start(self, path="eval.mp4", fps=30, H=256, W=256, camera_name="agentview"):
        ...
        if not self._rec.get("on", False):
            print(f"⚠️ WARNING: OpenCV failed to open VideoWriter for {path}. Skipping video recording.")
            return
...
# ==========================================
    # PRE-LOAD GEMINI EXAMPLES (Do this ONCE!)
    # ==========================================
    print("📦 Pre-loading Gemini Few-Shot Examples into RAM...")
    ex1_img = Image.open("eval_failed_cases/trial_33_FAILED_prediction_plot.png").copy()
    ex2_img = Image.open("eval_failed_cases/trial_23_FAILED_prediction_plot.png").copy()
    ex3_img = Image.open("eval_failed_cases/trial_19_FAILED_prediction_plot.png").copy()
    ex4_img = Image.open("eval_failed_cases/trial_4_SUCCESS_prediction_plot.png").copy()
    ex5_img = Image.open("eval_failed_cases/trial_35_SUCCESS_prediction_plot.png").copy()
    ...
    for i in range(cfg.num_times):
        ...
        try:
            ...
            video_env.video_stop() # Clean up video on failure so the file isn't corrupted
            continue
        ...
        if not is_safe:
            ...
            try:
                # Use a 'with' block to automatically close the file after uploading!
                with Image.open("debug_intervention/vis_episode_00000.png") as live_img:
                    prompt_contents = [
                        SYSTEM_RULES,
                        "\n--- EXAMPLE 1 ---",
                ...
                if not corrections:
                    ...
                    # 1. APPLY VLM CORRECTIONS TO A2 (If any exist)
                    if osc_delta:
                        ...
                        print(f"🔧 Applied VLM Nudges to A2: {osc_delta}")

                # 2. ALWAYS ENFORCE ALIGNMENT (Outside the if osc_delta block!)
                # This guarantees A1 matches A2's final state (whether A2 was VLM-corrected or just baseline)
        ...
        # ==========================================
        # --- START THE VIDEO HERE (After all Matplotlib/Gemini stuff is done) ---
        ...
        # Remove the patch and finalize the video for this trial
