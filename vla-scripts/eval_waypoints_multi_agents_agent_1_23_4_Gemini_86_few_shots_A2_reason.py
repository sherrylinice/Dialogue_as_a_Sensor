...
Because the orientation is good but it barely misses the right edge, I will use the "Micro-Nudge XY" strategy. A tiny shift left (-0.02, or -0.03) will center A2.
...
```'''
    ...
    for i in range(cfg.num_times):
        ...
        # ==========================================
        # MULTI-AGENT RECOVERY SYSTEM (MULTI-TURN REFLECTION)
        # ==========================================
        MAX_RETRIES = 2  # Set to 2 to give it a second chance if the first fix fails
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if is_safe:
                if attempt > 0:
                    print("\n✅ Safety check PASSED after VLM correction! Executing...")
                break # Exit the reflection loop, trajectory is safe!

            print("\n" + "!"*50)
            print(f"🛑 AGENT 1 TRIGGER (Attempt {attempt+1}/{MAX_RETRIES}): SIMULATION PAUSED")
            ...
            # This generates the image with the CURRENT (potentially already corrected) waypoints
            ...
            # --- NEW: SAVE A UNIQUE COPY OF THE OVERLAY IMAGE FOR THIS TURN ---
            import shutil
            turn_image_path = f"debug_intervention/trial_{i}_attempt_{attempt}.png"
            shutil.copy("debug_intervention/vis_episode_00000.png", turn_image_path)
            
            print(f"\n🤖 Requesting Gemini Correction (Turn {attempt+1})...")
            ...
            try:
                ...
                if attempt == 0:
                    scenario_text = f"REAL SCENARIO - ATTEMPT 1\nBased on the rules and examples above, analyze this image, explicitly answer the 3-step checklist, and provide the JSON correction.\nTrigger: {trigger_reason}"
                else:
                    scenario_text = f"REAL SCENARIO - ATTEMPT {attempt+1} (REFLECTION)\nWARNING: Your previous correction was applied, but the trajectory is STILL UNSAFE. The safety system triggered again:\nTrigger: {trigger_reason}\nLook closely at this newly updated image, explicitly answer the 3-step checklist, and provide a NEW JSON correction."
                ...
                # Call the model (Strict JSON mode removed)
                ...
                # --- NEW PARSER: Extract JSON from markdown block ---
                # This finds everything between ```json and ```
                json_match = re.search(r'```json\s*(.*?)\s*```', vlm_raw_output, re.DOTALL)
                
                if json_match:
                    json_str = json_match.group(1)
                else:
                    # Fallback in case it didn't use markdown tags
                    json_str = vlm_raw_output.strip()
